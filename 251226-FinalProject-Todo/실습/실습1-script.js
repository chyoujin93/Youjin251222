// **[미션]** 주석에 적힌 6가지 미션을 수행하여 코드를 완성하세요.
...
// 로컬 스토리지에는 데이터가 문자열로만 저장되어있음.
// 그래서, 원래의 배열 형태로 변경해야함.
...
// 3. 저장 기능
function save() {
// [미션 2] 로컬 스토리지에 'myTodos' 이름으로 
// todoData 저장 (JSON 변환 필수)
// 배열->문자열로 변환해서, 로컬스토리지에 저장한다.
...
// [미션 6] 검색 기능 구현하기
// 1) todoData 배열에서 item.text에 keyword가 '포함'된 것만 filter 하세요. 
// (힌트: includes() 사용)
// ▼정답1
// const filteredData = todoData.filter
// (item => item.text.includes(keyword));

// ▼정답2_사용자의 편의성을 높이기 위해
// 대소문자 구분을 무시하는 기능을 추가한, 더 발전된 형태임.
const filteredData = todoData.filter
(item => item.text.toLowerCase().includes(keyword.toLowerCase()));

// 2) 걸러진 데이터(filteredData)만 화면에 그립니다. 
// (주의: 저장은 하지 않음)
render(filteredData);
});
